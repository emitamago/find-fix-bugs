...
describe("GET /users/[username]", function () {
  ...
  // TEST BUG 2
  test("should raise 404 error if no matching user found", async function () {
    const response = await request(app)
      .get("/users/hello")
      .send({ _token: tokens.u1 });
...
describe("PATCH /users/[username]", function () {
  ...
  // TEST BUG 3 -----this test passed with different reason. Add extra test 
  // to make sure raise error if not allowed fields is passed in
  test("should disallowing patching not-allowed-fields", async function () {
    ...
    expect(response.statusCode).toBe(400);
    // ADDED TEST BELOW
    expect(response.body.message).toBe(`Not allowed Fields`)
  });

  // TEST BUG 4
  test("should non admin user can patch own data", async function () {
    const response = await request(app)
      .patch("/users/u1")
  ...
  test("should return 401 if non admin user try to update other users", async function () {
    const response = await request(app)
      .patch("/users/u2")
      .send({ _token: tokens.u1, last_name: "new-ln1" });
    expect(response.statusCode).toBe(401);
...
describe("DELETE /users/[username]", function () {
  ...
  // TEST BUG 6
  test("should return error if no matching users", async function () {
    const response = await request(app)
      .delete("/users/hello")
      .send({ _token: tokens.u3 }); // u3 is admin
    expect(response.statusCode).toBe(404);
